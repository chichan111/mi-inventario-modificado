...
// Nueva función para exportación de inventario
if(!empty($_POST['action']) && $_POST['action'] == 'exportInventory') {
    $data = $inventory->exportInventory($_POST['format']);
    echo json_encode($data);
}

// Nueva función para exportación de órdenes
if(!empty($_POST['action']) && $_POST['action'] == 'exportOrders') {
    $data = $inventory->exportOrders($_POST['format']);
    echo json_encode($data);
}

// Actualizar lista de productos
if(!empty($_POST['btn_action']) && $_POST['btn_action'] == 'refreshProductList') {
    echo $inventory->productDropdownList();
}
...
// Stock verification endpoint
if(!empty($_POST['action']) && $_POST['action'] == 'verifyStock'){
    $response = $inventory->verifyStock($_POST['product_id'], $_POST['quantity']);
